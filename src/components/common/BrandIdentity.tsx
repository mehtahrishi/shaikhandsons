...
export const BrandIdentity = ({ collapsed = false, size = "md", className = "" }: BrandIdentityProps) => {
  return (
    <div className={cn("flex items-center gap-2 group whitespace-nowrap overflow-visible transition-all duration-300", className)}>
      {collapsed ? (
        ...
        <>
          <span className={cn(
            ...
            size === "sm" ? "text-xs" : size === "md" ? "text-sm md:text-lg" : size === "lg" ? "text-lg md:text-2xl" : "text-2xl md:text-3xl"
          )}>
            SHAIKH
          </span>
          <span className={cn(
