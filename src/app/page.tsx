...
export default function Home() {
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* From Top Brands Section */}
      {brandGroups.length > 0 && (
        <section className="pb-16 md:pb-20 bg-muted/5 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-border/50 to-transparent" />

          <div className="container mx-auto px-6">
            <div className="space-y-12 md:space-y-16">
              {brandGroups.map((group) => (
                <div key={group.id} className="space-y-4 md:space-y-5">
                  <div className="flex items-end justify-between border-b border-border/30 pb-3">
                    <div className="flex items-center gap-4 md:gap-6">
                      {group.imageUrl && (
                        <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl bg-card border border-border/50 p-2.5 flex items-center justify-center shadow-sm">
                          <img src={getImageUrl(group.imageUrl)} alt={group.name} className="max-w-full max-h-full object-contain" />
                        </div>
                      )}
                      <div>
                        <h3 className="text-xl md:text-2xl font-headline font-black uppercase tracking-tight leading-none">{group.name}</h3>
                        <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mt-1.5">
                          {group.vehicles.length} Models Available
                        </p>
                      </div>
                    </div>
                    <Link href={`/vehicles?brand=${group.id}`}>
                      <Button variant="ghost" className="text-[9px] font-black uppercase tracking-widest group px-0 h-auto hover:bg-transparent mb-0.5">
                        View All <ChevronRight className="ml-1 h-2.5 w-2.5 group-hover:translate-x-1 transition-transform" />
                      </Button>
                    </Link>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
                    {group.vehicles.map((vehicle: any) => (
                      <div key={vehicle.id} className="h-[440px] md:h-[460px]">
